...
/** 
  @description Gets the number of functions that have been called so far.  
               This corresponds to a stack level which can be used to determine
               the number of tabs to output on messages.

  @param {Array(String)} stack Array of functions previously called.  

  @returns {Number} Number of levels in the stack (i.e. size of the stack)
*/
...
/** 
  @description Creates the message that denotes the start of a function.  Also adds the function name to the stack.

  @param {String} name Function name
  @param {String} [mod] Module name
  @param {String} [user] User name
  @param {String} [time] Date/time stamp
  @param {Array(String)} stack Stack containing all previous function names

  @returns {String} Properly formatted string containing all relevant information pass through inputs.
*/
function start(name, mod, user, time, stack) {
  ...
  time = util.nvl(time, new Date().toString());

  // create the start message
  result = generateLogPrefix(level, mod, user, time, 'START');
  result += util.inow(name) ? 'anonymous' : name;
...
function end(name, mod, user, time, stack) { 
  log({msg: 'start end', internal: internal});
  var result, level;
  ...
  log({msg: 'end end', internal: internal});
  return result;
}
...
/**
  @description Logs the specified message to the 
*/
function log(ops) {
  var dops, aops;
  ...
  // check if input is simple string
  if (util.isString(ops)) {
    // if it is assume that it is the message we want to send out
    ops = { msg: ops };
  }
  aops = params(dops, ops);


  // only display messages when output is enabled
  if (aops.internal.output.on) {
    // call appropriate output method to display message
    // based on the value of aops.ops
    if (util.inou(aops.ops)) {
      // if we have specified aops.ops values then pass them along
      aops.internal.output.method(aops.msg);
    } else {
      // if we have not specified aops.ops values then ignore them
      aops.internal.output.method(aops.msg,aops.ops);
    }
  }
}
