...
function alreadyCalled(name, stack) {
  ...
  // matches the current name, then yes, we have
  // already called it
  if (current === name) {
...
function generateLogPrefix(msg, mod, user, time, action) {
  var level, spacing, result;

  level = getLevel(stack);
  spacing = tabs({count: level, start: tab});
...
function start(name, mod, user, time) {
  ...
  // create the start message
  result = generateLogPrefix(name, mod, user, time, 'START');
...
*/
function end(name, mod, user, time) { 
  ...
  // create the start message
  result = generateLogPrefix(name, mod, user, time, 'END');
